...
/**
 * A IMPRESSÃO DA LISTA DE IDS — pra foto saber de quem ela é foto.
 *
 * A foto era guardada por CONTRATO, e o contrato não muda quando a lista muda.
 * Quando os 1/1 passaram de 107 pra 159, a foto velha continuou "válida" e
 * respondia zero pros 52 novos donos — negando o cargo sem que nada parecesse
 * errado, porque negar é uma resposta legítima e o número de chamadas não
 * mudou. O `atualizaLords` ainda se recusava a refazê-la ("foto ainda vale"),
 * então o erro duraria as 3 horas inteiras a cada mexida na lista.
 *
 * Agora a foto carrega a impressão da lista que a gerou. Lista diferente,
 * impressão diferente, foto descartada na hora.
 *
 * FNV-1a: a lista é ordenada antes, pra reordenar não trocar a impressão.
 */
export function impressaoDosIds(ids) {
  const lista = [...(ids || [])].map(Number).sort((a, b) => a - b)
  let h = 0x811c9dc5
  for (const texto of lista.map((n) => String(n) + ',')) {
    for (let i = 0; i < texto.length; i++) {
      h ^= texto.charCodeAt(i)
      h = Math.imul(h, 0x01000193) >>> 0
    }
  }
  return `${lista.length}:${h.toString(16)}`
}
...
async function medeUma(carteira, r, jaMedido, opcoes) {
  ...
  if (tipo === 'rank') {
    ...
    /*
     * A MEDIDA É 0 OU 1, e nunca o rank em si. Ver `minimoDaRegra`: guardar o
     * rank e inverter a comparação pra `<=` faria carteira SEM rank virar zero,
     * e zero passa em qualquer "top N".
     *
     * `rank == null` é a carteira que não pontua — a API devolve nulo de
     * propósito, porque "sem classificação" e "em último lugar" são fatos
     * diferentes. Aqui os dois dão no mesmo: fora do top.
     */
    const dentro = typeof s.rank === 'number' && s.rank >= 1 && s.rank <= r.minimo
    return { ok: true, valor: dentro ? 1 : 0 }
  }
  ...
  if (tipo === 'ERC-721-ids') {
    ...
    if (mapa && mapa.contrato === contrato && mapa.completo && mapa.impressao === impressaoDosIds(r.ids)) {
...
export async function atualizaLords(regras) {
  ...
  const impressao = impressaoDosIds(regra.ids)
  const atual = await leLords()
  if (
    atual &&
    atual.contrato === contrato &&
    atual.completo &&
    atual.impressao === impressao &&
    Date.now() - atual.quando < VALIDADE_LORDS
  ) {
   ...
   */
  await guardaLords({ ...mapa, contrato, impressao })
  return {
