 ...
 *                         pontuação de corte.
 *
 *   'rank'                A POSIÇÃO da carteira no ranking. `minimo` é o N de
 *                         "top N". Também sem contrato.
 *
 *                         Existe separado do score porque as duas perguntas
 *                         envelhecem diferente: um corte de 2.320 pontos é "top
 *                         69" hoje e outra coisa daqui a três meses, conforme a
 *                         pontuação da população inteira sobe. "Top 69" continua
 *                         sendo top 69. Escolher entre as duas é escolher entre
 *                         premiar um feito ou premiar uma posição.
 *
...
export function chaveDaMedida(r) {
  ...
  /*
   * O N ENTRA NA CHAVE porque top 69 e top 500 são perguntas diferentes.
   * Compartilhar a resposta faria a segunda regra herdar o veredito da primeira.
   */
  if (tipo === 'rank') return `rank:${r?.minimo}`
...
export function minimoDaRegra(r) {
  const tipo = tipoDaRegra(r)
  /*
   * POSSE-DE-ID E RANK MEDEM SIM/NÃO, e por isso o corte é sempre 1.
   *
   * No caso do rank isso não é detalhe, é a defesa principal. Rank é INVERTIDO —
   * 10 é melhor que 500 —, e a comparação deste arquivo é uma só: `medida >=
   * corte`. Guardar o rank como medida e inverter a comparação pra `<=` parece
   * natural e é o buraco: `medidas[chave] ?? 0` faz carteira SEM rank virar
   * zero, e `0 <= 69` é verdadeiro. Todo mundo que nunca pontuou ganharia o
   * cargo mais exclusivo do servidor.
   *
   * Com medida 0/1 a comparação nunca inverte, e o ausente continua sendo o não.
   * O N vive na chave da medida (ver `chaveDaMedida`), não aqui.
   */
  return tipo === 'ERC-721-ids' || tipo === 'rank' ? 1 : r?.minimo
}
...
export function validaRegras(regras) {
  ...
  for (const [i, r] of regras.entries()) {
    ...
    const ehRank = tipo === 'rank'
    // Score e rank saem do mesmo lugar (a API de analytics) e nenhum dos dois
    // tem contrato.
    const ehScore = tipo === 'score' || ehRank
    ...
    if (ehScore) {
      if (r.contrato) {
        erros.push(`regra ${i + 1}: regra de ${ehRank ? 'rank' : 'score'} não tem "contrato"`)
      }
