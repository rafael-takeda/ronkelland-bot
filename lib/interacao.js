...
export async function decide(corpo, { segredo }) {
  ...
  if (ehVerify) {
    ...
    /*
     * DUAS MENSAGENS, E O ENDEREÇO SOZINHO NA SEGUNDA.
     *
     * `depois` roda DEPOIS da resposta ir — o Discord só aceita mensagem de
     * acompanhamento em interação já respondida. Ver `msg.soOEndereco` pro
     * motivo de ela existir: bloco de código no meio de um texto longo não se
     * copia no celular.
     *
     * Se esta segunda mensagem falhar, a primeira já foi e continua explicando
     * tudo — a pessoa perde a facilidade de copiar, não a verificação.
     */
    return {
      resposta: respondeSo(msg.comoVerificar(endereco, JANELA_MIN, jaTem)),
      depois: () => avisaDepois(corpo.application_id, corpo.token, msg.soOEndereco(endereco)),
