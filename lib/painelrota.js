...
// Ver `regrasEmUso`: importado, e nao lido do disco, pra que o empacotador seja
// obrigado a incluir o arquivo.
import arquivo from '../regras.json' with { type: 'json' }
...
export async function regrasEmUso() {
  ...
  /*
   * IMPORT, E NÃO `readFileSync`.
   *
   * Ler do disco depende de o empacotador ter copiado o arquivo junto da função
   * E de `import.meta.url` apontar pro lugar certo depois do empacotamento —
   * duas coisas que não se controlam daqui. Quando falha, `regras` vira lista
   * vazia, e lista vazia não estoura: ela responde "você não tem cargo nenhum" a
   * quem tem tudo.
   *
   * Import faz o arquivo virar parte do grafo de módulos. O empacotador não tem
   * como não incluir uma coisa que o código importa.
   */
  return arquivo.regras ?? []
}
...
export async function respondePainel(corpo, membro) {
  ...
  if (acao === 'add:tipo') {
    ...
    /*
     * Score passa por uma tela antes, com a régua do dia. Um campo "score
     * mínimo" vazio não diz se 500 é muito ou pouco, e o chute erra pros dois
     * lados: cargo inalcançável ou cargo que todo mundo tem.
     */
    return { resposta: mostra(telaDoScore(await marcosDeCorte())) }
  }
