 ...
 */
import { cargosDoMembro, cargosDoServidor, postaNoCanal } from './discord.js'
...
/**
 * Os cargos do servidor e a posição do bot, numa ida só.
 *
 * `posicaoDoBot` já busca a lista de cargos por dentro pra achar a posição —
 * pedir os dois separados buscaria a mesma lista duas vezes, e isto roda dentro
 * dos 3 segundos que o Discord dá pra responder.
 *
 * Deu erro? Devolve `cargos: null`, e o seletor cai no nativo do Discord: lista
 * pior, mas o admin não fica travado por causa de uma chamada que falhou.
 */
async function cargosPraEscolher(servidor, idDoBot) {
  try {
    const [cargos, meus] = await Promise.all([
      cargosDoServidor(servidor),
      cargosDoMembro(servidor, idDoBot),
    ])
    const tenho = new Set(meus)
    const posicao = cargos.filter((c) => tenho.has(c.id)).reduce((alto, c) => Math.max(alto, c.position), 0)
    return { cargos, posicao }
  } catch {
    return { cargos: null, posicao: 0 }
  }
}
...
export async function respondePainel(corpo, membro) {
  ...
  if (acao === 'add:rank:janelinha') {
    ...
    const { cargos, posicao } = await cargosPraEscolher(servidor, corpo.application_id)
    return {
      resposta: abre(escolheCargo(`Top **${minimo.toLocaleString('en-US')}** by Ronke Score`, cargos, posicao)),
    }
  }
  ...
  if (acao === 'add:score:janelinha') {
    ...
    const { cargos, posicao } = await cargosPraEscolher(servidor, corpo.application_id)
    return {
      resposta: abre(escolheCargo(`Ronke Score **${minimo.toLocaleString('en-US')}** or higher`, cargos, posicao)),
    }
  }
  ...
  if (acao === 'add:janelinha') {
    ...
    return {
      resposta: abre(
        escolheCargo(`\`${contrato}\`\nMinimum: **${minimo.toLocaleString('en-US')}**`, cargos, posicao),
      ),
    }
  }
  ...
  if (acao === 'add:cargo') {
    ...
    const [{ cargos, posicao }, regras] = await Promise.all([
      cargosPraEscolher(servidor, corpo.application_id),
      regrasEmUso(),
    ])

    /*
     * A lista montada aqui é de texto, e lista de texto manda só o valor — o
     * cargo vem da lista que a gente já buscou pra saber a posição do bot, sem
     * chamada a mais. O `resolved` é o caminho do seletor nativo, que ainda roda
     * quando a busca de cargos falha.
     */
    const cargo = cargos?.find((c) => c.id === idCargo) || corpo.data?.resolved?.roles?.[idCargo]
    if (!cargo) return { resposta: so('That role no longer exists — start again with `/ronkelland`.') }
  ...
  if (acao === 'rm') {
    /*
     * O NOME VEM DO SERVIDOR, e não do que foi guardado no cadastro.
     *
     * Quem renomeia um cargo no Discord não atualiza o `nome` da regra — e nome
     * velho numa lista de REMOÇÃO é o pior lugar pra ele estar: a pessoa apaga
     * achando que é outro cargo. No painel isso se resolve mencionando o cargo;
     * aqui não dá, porque rótulo de opção é texto puro.
     */
    const [regras, cargos] = await Promise.all([
      regrasEmUso(),
      cargosDoServidor(servidor).catch(() => null),
    ])
    const nomeAgora = cargos ? Object.fromEntries(cargos.map((c) => [c.id, c.name])) : null
    return { resposta: mostra(escolheParaRemover(regras, nomeAgora)) }
  }
  ...
  if (acao === 'rm:qual') {
    ...
    return {
      resposta: await painelAtual(
        // Menção, e não `fora.nome`: se o cargo foi renomeado desde o cadastro, o
        // nome guardado confirmaria a remoção do cargo errado.
        `Removed <@&${fora.cargo}>. Members keep it until the next check.`,
        false,
      ),
