...
const CH = {
  ...
  lords: 'rl:lords', // foto de quem tem cada 1/1, tirada quando ninguem espera
}
...
/**
 * RESERVA O TRABALHO DE UM ENDEREÇO. `SET NX` é atômico: ou você pega, ou não.
 *
 * Dois cliques seguidos (e a pessoa impaciente clica) fariam duas verificações
 * simultâneas do mesmo pagamento — duas `amarra`, dois `aplicaCargos`, duas
 * mensagens. Quem não pega a reserva sai calado dizendo "ainda estou olhando".
 *
 * Expira sozinha: se a invocação morrer no meio, ninguém fica travado pra
 * sempre. É por isso que é `EX` e não uma chave que alguém tem que lembrar de
 * apagar.
 */
export async function reserva(endereco, quem, segundos) {
  const r = await cmd('SET', `rl:trab:${endereco.toLowerCase()}`, quem, 'NX', 'EX', String(segundos))
  return r === 'OK'
}

export async function liberaReserva(endereco) {
  await cmd('DEL', `rl:trab:${endereco.toLowerCase()}`)
}

/**
 * O TETO DIÁRIO DE APERTOS, por membro.
 *
 * Cada clique custa consulta ao explorer, ao RPC e ao Discord. O plano gratuito
 * da Vercel não cobra quando estoura — ele BLOQUEIA a função por 30 dias. Não é
 * fatura, é apagão: o bot inteiro para, pra todo mundo.
 *
 * Trinta apertos por dia é muito mais do que qualquer uso honesto (verificar são
 * dois ou três) e pouco o bastante pra ninguém derrubar nada sozinho.
 *
 * O `EXPIRE` só no primeiro `INCR` — reaplicar a cada aperto faria a janela
 * andar pra frente e o contador nunca zerar.
 */
export async function gastou(membro, teto) {
  const chave = `rl:apertos:${membro}`
  const n = Number(await cmd('INCR', chave))
  if (n === 1) await cmd('EXPIRE', chave, '86400')
  return n > teto
}
...
/**
 * A foto de quem tem cada 1/1. Ver `mapaDeDonos` em `prova.js`.
 *
 * Guardada com o CONTRATO junto: se a coleção mudar de endereço, um mapa velho
 * apontando pro contrato antigo responderia com confiança sobre a coisa errada.
 */
export async function guardaLords(mapa) {
  await cmd('SET', CH.lords, JSON.stringify(mapa))
}

export async function leLords() {
  const v = await cmd('GET', CH.lords)
  if (!v) return null
  try {
    const m = JSON.parse(v)
    return m && typeof m.donos === 'object' ? m : null
  } catch {
    // Mapa corrompido vira "nao tenho mapa", e nunca "mapa vazio". Vazio diria
    // que ninguem tem 1/1 -- e num mapa que so serve pra NEGAR, isso tiraria o
    // cargo mais raro do servidor de todo mundo de uma vez.
    return null
  }
}
