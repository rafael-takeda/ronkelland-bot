...
/**
 * OS MARCOS DE CORTE — quanto vale cada faixa, hoje.
 *
 * Existe por um problema concreto do painel: "score mínimo" é uma caixa de texto
 * vazia, e ninguém sabe se 500 é muito ou pouco. Sem referência, o admin chuta —
 * e um chute alto cria um cargo que ninguém alcança, um chute baixo cria um
 * cargo que todo mundo tem. Os dois matam o cargo, de maneiras opostas.
 *
 * O truque é pedir UMA linha do ranking em cada posição, em vez de baixar as
 * 6.200: cinco chamadas minúsculas em paralelo, ~200 ms no total.
 *
 * DEVOLVE O RANK QUE A API DIZ, não o que foi pedido. Com empates, a posição 100
 * pode responder "rank 99" — e mostrar 100 seria inventar precisão.
 */
export async function marcosDeCorte(posicoes = [10, 50, 100, 500, 1000]) {
  const linhas = await Promise.all(
    posicoes.map(async (p) => {
      try {
        const j = await pega(`/leaderboard?limit=1&offset=${p - 1}`)
        const e = j?.data?.entries?.[0]
        if (!e || typeof e.score !== 'number') return null
        return { rank: e.rank ?? p, score: e.score }
      } catch {
        return null
      }
    }),
