...
export const COMANDOS = [
  ...
  {
    name: 'ronkelland',
    description: 'Set up which token grants which role',
    default_member_permissions: MANAGE_GUILD,
    options: [
      {
        type: TIPO.GRUPO,
        name: 'rule',
        description: 'Manage the token gating rules',
        options: [
          {
            type: TIPO.SUB,
            name: 'add',
            description: 'Grant a role to holders of a token',
            options: [
              {
                type: TIPO.STRING,
                name: 'contract',
                description: 'Contract address on Ronin (0x…)',
                required: true,
              },
              {
                type: TIPO.INTEGER,
                name: 'minimum',
                description: 'How many they must hold',
                required: true,
                min_value: 1,
              },
              {
                /*
                 * TIPO.ROLE é o que faz o Discord desenhar a LISTA de cargos do
                 * servidor. Sem ele a pessoa teria que colar o ID na mão — que é
                 * exatamente o problema que este comando existe pra resolver.
                 */
                type: TIPO.ROLE,
                name: 'role',
          ...
          },
          {
            type: TIPO.SUB,
            name: 'list',
            description: 'Show the rules currently set up',
            options: [],
          },
          {
            type: TIPO.SUB,
            name: 'remove',
            description: 'Delete a rule (holders lose the role on the next check)',
            options: [
              {
                type: TIPO.ROLE,
                name: 'role',
                description: 'Role whose rule you want to delete',
                required: true,
              },
            ],
          },
