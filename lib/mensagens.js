...
export const msg = {
   ...
   */
  comoVerificar: (endereco, minutos, jaTem, separado) => {
    const cabeca = jaTem
      ? [
          '**Re-check your roles**',
          '',
  ...
  /**
   * O ENDEREÇO SOZINHO, quando dá pra mandar uma segunda mensagem.
   *
   * Um bloco de código que é a única coisa da mensagem ganha o botão de copiar
   * no Discord móvel — e copiar 42 caracteres à mão no celular é onde o fluxo
   * mais perde gente.
   *
   * É conveniência, não informação: o endereço já está no topo da mensagem
   * principal. Se esta não sair, ninguém fica sem saber pra onde mandar.
   */
