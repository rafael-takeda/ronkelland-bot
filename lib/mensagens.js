...
export const msg = {
  ...
  comoVerificar: (endereco, minutos, jaTem) => {
    ...
    return [
      ...cabeca,
      '',
      'Send a transaction from the wallet that holds your NFT to the address in the **next message**.',
      '',
      /*
  ...
  /**
   * O ENDEREÇO SOZINHO, NUMA MENSAGEM SÓ DELE.
   *
   * Ele já estava na instrução, num bloco de código no meio do texto. No
   * computador dá pra selecionar; no CELULAR é quase impossível — o toque longo
   * pega o parágrafo inteiro, e a pessoa acaba colando texto no lugar do
   * endereço, ou digitando 42 caracteres à mão.
   *
   * Numa mensagem em que o bloco de código é a ÚNICA coisa, o Discord móvel
   * mostra o botão de copiar. Some o problema inteiro.
   *
   * Por isso são duas mensagens e não uma, e por isso esta vem DEPOIS: ela fica
   * embaixo, que é onde o polegar está e onde o olho vai primeiro.
   */
  soOEndereco: (endereco) => '```\n' + endereco + '\n```',
